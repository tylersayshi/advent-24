...
// only need to call invalid when b comes before a when a is present

// get the valid rows then get the middle number of each row and sum them

const rowIsValid = (
  row: number[],
  beforeAfter: Record<string, Set<number>>
) => {
  const seen = new Set<number>();
  let valid = true;
  for (const num of row) {
    const numsThatMustBeBefore = beforeAfter[num];
    if (numsThatMustBeBefore) {
      const invalidSet = seen.intersection(numsThatMustBeBefore);
      if (invalidSet.size) {
        valid = false;
        break;
      }
...
// const getInputMiddleSum = (input: string) => {
//   const [rules, updates] = input.split("\n\n");

//   const beforeAfter = rules
//     .split("\n")
//     .reduce<Record<number, Set<number>>>((acc, ruleRow) => {
//       const [a, b] = ruleRow.split("|").map(Number);
//       if (a in acc) {
//         acc[a].add(b);
//       } else {
//         acc[a] = new Set([b]);
...
//     if (rowIsValid(update, beforeAfter)) {
//       validRows.push(update);
//     }
//   }
//   return validRows.reduce<number>((acc, row) => {
//     const rowNums = row.split(",").map(Number);
//     const middleIndex = Math.floor(rowNums.length / 2);
//     if (rowNums.length / 2 === 0) {
//       throw new Error("EVEN");
//     }
//     return acc + rowNums[middleIndex];
//   }, 0);
// };
...
const getInputMiddleSumP2 = (input: string) => {
  const [rules, updates] = input.split("\n\n");

  const afterBefore = rules
    .split("\n")
    .reduce<Record<number, Set<number>>>((acc, ruleRow) => {
      const [a, b] = ruleRow.split("|").map(Number);
      if (b in acc) {
  ...
  for (const rule of rules.split("\n")) {
    ...
    // problem is when a or b are already in the list we need to reorder them

    if (!orderedNums.length) {
      orderedNums = orderedNums.concat([a, b]);
      continue;
    }

    if (orderedNums.includes(a) && orderedNums.includes(b)) {
      const aIndex = orderedNums.indexOf(a);
      const bIndex = orderedNums.indexOf(b);
      // remove and reinsert
      if (aIndex > bIndex) {
        orderedNums = [
          ...orderedNums.slice(0, bIndex),
          ...orderedNums.slice(bIndex + 1, aIndex),
          ...orderedNums.slice(aIndex + 1),
        ];
        // console.log("removed", { a, b }, orderedNums);
      }
    }

    if (!orderedNums.includes(b)) {
      const seen = [];
      const mustComeAfter = afterBefore[b]!;
      if (!mustComeAfter) {
        throw new Error("ahhh");
      }

      let i = 0;
      while (seen.length < mustComeAfter.size && i !== orderedNums.length) {
        if (mustComeAfter.has(orderedNums[i])) {
          seen.push(orderedNums[i]);
        }
        i++;
      }
      if (!i) {
        orderedNums.unshift(b);
      } else if (i === orderedNums.length) {
        orderedNums.push(b);
      } else {
        orderedNums = [...orderedNums.slice(0, i), b, ...orderedNums.slice(i)];
      }
    }

    if (!orderedNums.includes(a)) {
      // b must be in there and it must come before
      // start just before b, continue until preceding slice does not include anything a must be before
      const aMustBeBefore = beforeAfter[a];
      let bIndex = orderedNums.findIndex((n) => n === b);

      while (
        new Set(orderedNums.slice(0, bIndex)).intersection(aMustBeBefore)
          .size &&
        !!bIndex
      ) {
        bIndex--;
      ...
      if (bIndex === 0) {
        orderedNums.unshift(a);
      } else {
        orderedNums = [
          ...orderedNums.slice(0, bIndex),
          a,
          ...orderedNums.slice(bIndex),
        ];
      }
    }
  }

  // second pass to try to correct order

  // for (let i = 0; i < orderedNums.length; i++) {
  //   const num = orderedNums[i]!;
  //   const numMustBeAfter = beforeAfter[num];
  //   let indForNum = i;
  //   while (
  //     new Set(orderedNums.slice(0, indForNum)).intersection(numMustBeAfter)
  //       .size &&
  //     indForNum !== orderedNums.length
  //   ) {
  //     indForNum++;
  //   }
  //   if (indForNum !== i) {
  //     orderedNums = [
  //       ...orderedNums.slice(0, i),
  //       ...orderedNums.slice(i + 1, indForNum),
  //       num,
  //       ...orderedNums.slice(indForNum),
  //     ];
  //   }
  // }
  ...
  for (let i = 0; i < orderedNums.length; i++) {
    const num = orderedNums[i]!;
    const numsThatMustBeBefore = afterBefore[num];
    let indForNum = orderedNums.length - 1 - i;
    while (
      new Set(orderedNums.slice(indForNum)).intersection(numsThatMustBeBefore)
        .size &&
      indForNum !== 0
    ) {
      indForNum--;
    ...
    if (!indForNum) {
      orderedNums = orderedNums.filter((n) => n !== num);
      orderedNums.unshift(num);
    } else if (indForNum !== i) {
      orderedNums = [
        ...orderedNums.slice(0, i),
        ...orderedNums.slice(i + 1, indForNum),
  ...
  const invalidRows = updates.split("\n").filter((r) => !validRows.includes(r));
  const finalRows: string[] = [];

  for (const row of invalidRows) {
    const rowNums = row.split(",").map(Number);
    rowNums.sort((a, b) => orderedNums.indexOf(a) - orderedNums.indexOf(b));

    finalRows.push(rowNums.join(","));
  }

  let invalidCount = 0;
  for (const row of finalRows) {
    if (!rowIsValid(row.split(",").map(Number), beforeAfter)) {
      invalidCount++;
    }
  }
  console.log("invalid", invalidCount);

  const finalFinalRows = [];
  for (const row of finalRows) {
    const numRow = row.split(",").map(Number);
    while (!rowIsValid(numRow, beforeAfter)) {
      numRow.sort(() => Math.random());
    }
    finalFinalRows.push(numRow.join(","));
    console.log("rows added", finalFinalRows.length);
  }

  return finalFinalRows.reduce<number>((acc, row) => {
    const rowNums = row.split(",").map(Number);
    const middleIndex = Math.floor(rowNums.length / 2);
    if (rowNums.length / 2 === 0) {
      throw new Error("EVEN");
...
const getInputMiddleSumP2Try2 = (input: string) => {
  const [rules, updates] = input.split("\n\n");
  ...
  const processUpdates = (update: number[]): number[] => {
    const pageOrderMap = new Map<number, number>();

    for (let i = 0; i < update.length; i++) {
      const page = update[i];
      pageOrderMap.set(page, 0);
    }

    for (const [a, bSet] of Object.entries(beforeAfter)) {
      const aNum = Number(a);
      for (const b of bSet) {
        if (pageOrderMap.has(b) && pageOrderMap.has(aNum)) {
          pageOrderMap.set(b, pageOrderMap.get(b)! + 1);
        }
      }
    }

    const sortedEntries = Array.from(pageOrderMap.entries());

    sortedEntries.sort((entryA, entryB) => entryA[1] - entryB[1]);

    const sortedPages = [];
    for (let i = 0; i < sortedEntries.length; i++) {
      sortedPages.push(sortedEntries[i][0]);
    }

    return sortedPages;
  };

  const finalRows = updates
    .split("\n")
    .map((update) => {
      const pages = update.split(",").map(Number);
      if (!rowIsValid(pages, beforeAfter)) {
        // continuesly update scores
        return processUpdates(pages);
      }
      return null;
    })
    .filter(Boolean);

  return finalRows.reduce<number>((acc, row) => {
    const middleIndex = Math.floor(row!.length / 2);
    if (row!.length / 2 === 0) {
      throw new Error("EVEN");
    }
    return acc + row![middleIndex];
  }, 0);
};

console.log(getInputMiddleSumP2Try2(realInput));
